    ...
    <div class="col-xl-9 mx-auto">
         <div class="card thm-card  assign-campaign-container">
            <div class="card-header thm-card-head bg-gradient-danger d-flex flex-wrap align-items-center justify-content-between">
                ...
                <a href="{{route('campaign.list')}}" class="btn btn-light btn-sm text-primary">Back<i class="fa fa-arrow-left ms-1"></i></a>
            </div>
<!-- <div class="assign-campaign-container">
    <h4 class="heading">Assign Campaign: {{ $campaign->campaign_name }}</h4> -->
<div class="card-body thm-card-body ">
    <form action="{{ route('campaignAssign.vendor', $campaign->id) }}" method="POST" class="assign-form">
        @csrf
            ...
            <div class="city-list">
                @foreach($cities as $city)
                    <div class="form-check city-block d-flex align-items-center" style="margin-bottom: 5px;">
                        ...
                        <!-- Vendor Limit Input -->
                        <div class="fieldagent-limit" id="fieldagent_limit_{{ $loop->index }}" style="display: none; margin-left: 20px;">
                            <label for="limit_{{ $loop->index }}">Field Agent:</label>
                            <input type="number" name="fieldagent_limit[{{ $city }}]" id="limit_{{ $loop->index }}" placeholder="All" min="1" class="form-control form-control-sm ms-2" style="width: 80px; display: inline-block;">
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <button type="submit" class="btn-assign btn-primary">Assign to Field Agent</button>
    </form>
    </div>
</div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.city-checkbox').forEach((checkbox, index) => {
            checkbox.addEventListener('change', function () {
                const vendorLimit = document.getElementById(`fieldagent_limit_${index}`);
